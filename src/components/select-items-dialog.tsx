...
function chunkRows(items: SelectDialogItem[]): SelectDialogItem[][] {
  const grid: SelectDialogItem[][] = []
...
interface SelectItemsDialogProps {
  ...
  invalid?: boolean
  placeholder?: string
  /** Título del modal, ej. "Seleccionar áreas o dimensiones". */
  dialogTitle: string
  /** Placeholder del buscador, ej. "Buscar área...". */
  searchPlaceholder: string
  /** Ej. "No se encontraron áreas." */
  emptyMessage: string
  /** Ej. `(n) => n === 1 ? "1 área seleccionada" : \`${n} áreas seleccionadas\`` */
  selectedCountLabel: (count: number) => string
}

/**
 * Modal de selección múltiple con búsqueda y grilla paginada — la misma UI
 * que ya usa el selector de "área/dimensiones" de Referente curricular,
 * generalizada para cualquier catálogo `{id, label}` en vez de estar atada al
 * catálogo general de áreas MEN. Cada llamador trae sus propios `items`: no
 * asume de dónde vienen ni qué representan sus ids.
 */
export function SelectItemsDialog({
  ...
  const grid = chunkRows(pageItems)
  ...
  function toggle(itemId: number) {
    setDraft((prev) => (prev.includes(itemId) ? prev.filter((id) => id !== itemId) : [...prev, itemId]))
  }
  ...
  const selectedNames = value.map((itemId) => itemById.get(itemId)?.label).filter(Boolean) as string[]
  ...
  function removeSelected(itemId: number) {
          ...
          {selectedNames.length === 0 ? (
            ...
            <>
              {visibleChips.map((name, i) => {
                ...
                const itemId = value.find((id) => itemById.get(id)?.label === name)
                return (
                  <Badge
                    key={name}
                    variant="soft"
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="flex flex-col gap-4">
            ...
            {draft.length > 0 && (
              <p className="text-muted-foreground text-sm">{selectedCountLabel(draft.length)}</p>
            )}
