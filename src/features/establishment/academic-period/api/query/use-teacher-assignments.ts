...
// `GET /eval-col/asignaciones/docente/:ID` (`fn_asignacion_docente`, id_query
// 84 — corregido en V69, tenía academic_period_id/fk_funcionario invertidos).
// El `:ID` de la ruta es el funcionario (PK_TFUNCIONARIO); el periodo va por
// query string.
async function fetchTeacherAssignments(
  ...
  const raw: TeacherAssignmentsResponse = await api.get(
    `/eval-col/asignaciones/docente/${funcionarioId}?academicPeriodId=${academicPeriodId}`
  )
