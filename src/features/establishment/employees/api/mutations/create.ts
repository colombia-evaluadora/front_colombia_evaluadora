 ...
 * la resuelve `/auth/register/funcionario` (auth-center), que crea TUSUARIO +
 * TFUNCIONARIO de una vez — ver `registerFuncionario` y la rama
 ...
 * caía acá en vez de ir por `/register/funcionario`, mostrando un 404 en
  ...
  if (!env.ENABLE_API_MOCKING) {
    throw new Error(
      "POST /establishments/employees solo existe en el mock. En el backend real el alta " +
        "de funcionario va por /auth/register/funcionario (ver registerFuncionario).",
    )
  }
