...
function computePersonErrors(
  ...
  options: { establishmentId: number | null; activeEmployeeId: number | null; showEstablishmentPicker: boolean },
  ...
  if (
    ...
    !options.activeEmployeeId &&
    options.showEstablishmentPicker &&
...
export function ManageEmployeeDialog({ open, onOpenChange, employeeId }: ManageEmployeeDialogProps) {
  ...
  // Después de un intento de guardar fallido (`personErrors` ya tiene algo),
  // cada cambio vuelve a validar para que el mensaje/borde rojo de un campo
  // desaparezca apenas se completa, en vez de quedar pegado hasta el
  // siguiente click en "Guardar" — antes `personErrors` solo se recalculaba
  // ahí, así que escribir en un campo marcado no lo "limpiaba".
  useEffect(() => {
    if (Object.keys(personErrors).length === 0) return
    setPersonErrors(
      computePersonErrors(person, confirmPassword, {
        establishmentId,
        activeEmployeeId,
        showEstablishmentPicker,
      }),
    )
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `personErrors`
    // se lee solo como guard (no debe volver a disparar el efecto cuando el
    // propio `setPersonErrors` de acá lo cambia).
  }, [person, confirmPassword, establishmentId, activeEmployeeId, showEstablishmentPicker])
