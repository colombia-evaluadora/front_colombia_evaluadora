...
import type { GradeChangeKind, GradeChangeGradesAction } from "@/features/coverage/components/dialogs/dialog-grade-change"

const KIND_LABELS: Record<GradeChangeKind, string> = {
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl" showCloseButton={false}>
        ...
        <div className="rounded-md border border-input bg-muted/30 p-4">
          <p className="mb-1 text-sm font-semibold text-foreground">
            Resumen del movimiento - {KIND_LABELS[summary.kind]}
          </p>
          <ul className="flex list-disc flex-col gap-0 pl-5 text-sm text-foreground leading-tight">
            ...
            <li>
              ...
              {isSuperior ? "Cambio a grado superior" : "Cambio a grado inferior"}
            </li>
            <li>
              <span className="font-medium">Sede:</span> {summary.campus}
            </li>
            <li>
              <span className="font-medium">Grado:</span> {formatGrade(summary.fromGrade)} →{" "}
              <strong>{formatGrade(summary.toGrade)}</strong>
            </li>
            <li>
