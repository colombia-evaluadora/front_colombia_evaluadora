...
import {
  ...
  SHIFTS,
  enrollmentFiltersFormSchema,
  type EnrollmentFiltersFormInput,
  type EnrollmentFiltersFormValues,
...
interface FilterEnrollmentsFormProps {
  id: string
  defaultValues: EnrollmentFiltersFormInput
  onSubmit: (values: EnrollmentFiltersFormValues) => void
...
export function FilterEnrollmentsForm({
  id,
  defaultValues,
  onSubmit,
  catalogs,
  hideDocumentNumber = false,
}: FilterEnrollmentsFormProps) {
  const form = useAppForm({
    defaultValues,
    validators: {
      onSubmit: enrollmentFiltersFormSchema,
    },
  ...
  return (
    ...
    >
      ...
      {/* Fecha de inscripción: dos campos independientes (no un único rango),
          cada uno en modo `datetime` —calendario y hora en el mismo
          popover—. La leyenda sí aporta acá: "Desde" y "Hasta" sueltos no
          dicen de qué son. */}
      <FieldSet>
        <FieldLegend variant="label">Fecha de inscripción</FieldLegend>
        <div className="grid grid-cols-2 gap-x-4 gap-y-2">
...
function CheckboxFilterGroup<T extends string>({
   ...
   * acá el `Field` es el contenedor y la etiqueta va adentro.
   */
  return (
