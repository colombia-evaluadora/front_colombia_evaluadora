...
export interface CreateMatriculaBodyContext {
  sedeId: number
  jornadaId: number
  pkUsuarioEstudiante: number
  pkUsuarioAcudiente: number
}

export function toCreateMatriculaBody(
  values: CreateMatriculaInput,
  context: CreateMatriculaBodyContext,
) {
  ...
  return {
    SEDE: context.sedeId,
    JORNADA: context.jornadaId,
    GRUPO: toIntOrNull(academic.group),
    CARACTER_ESPECIALIDAD_ENFASIS: toIntOrNull(academic.specialty),
    PK_USUARIO_ESTUDIANTE: context.pkUsuarioEstudiante,
